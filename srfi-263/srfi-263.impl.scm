...
(define (messages-direct-lookup self name message-alist)
  (let ((slot-alist ((self 'mirror) 'immediate-message-alist)))
    (cond ((assq name slot-alist) => cdr)
          (else #f))))

(define (messages-recursive-lookup self name message-alist)
  (cond
      ...
      (let loop ((parents (mirror 'immediate-ancestor-list))
        ...
        (if (null? parents)
            ...
            (let-values (((new-receiver new-handler new-found)
                          (messages-recursive-lookup (car parents) name)))
              (loop (cdr parents)
...
(define-record-type message
  (make-message type getter-or-setter proc)
  message?
  (type message-type)
  (getter-or-setter message-getter-or-setter)
  (proc message-proc))

(define (messages-add! messages name message)
